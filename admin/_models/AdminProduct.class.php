...
class AdmincProdutos {
    ...
    public function ExeCreate(array $Data) {
       ...
       // var_dump($Data);
        if (in_array('', $this->Data)):
          ...
          if(Check::CodUnic($this->Data['cod_unico']) == TRUE)  :
                 
            $this->Error = ["Seu Código Único Já existe no banco de dados, favor use outro.!", WS_ALERT];
            $this->Result = false;
         
            else:
                  
             if ($this->Data['img_prod']):
                $uplaod = new Upload;
                $uplaod->Image($this->Data['img_prod'], $this->Data['prod_name']);
            endif;

            if (isset($uplaod) && $uplaod->getResult()):
                $this->Data['img_prod'] = $uplaod->getResult();
                $this->Create();
            else:
                $this->Data['img_prod'] = "images/2015/03/footer-logo.png";
                $this->Create();
            endif;
    ...
    private function setData() {
        ...
        $this->Data['cod_unico'] = Check::CodUnic($this->Data['cod_unico']);
    ...
    private function Create() {
        ...
        if ($cadastra->getResult()):
            $this->Error = ["O post {$this->Data['prod_name']} foi cadastrado com sucesso no sistema!", WS_ACCEPT];
